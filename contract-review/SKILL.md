...
Not for: **a standalone BAA or Subcontractor BAA — use `baa-review`.** That skill carries the
§ 164.504(e)(2) required-element map, the recurring BAA clause positions with proposed wording,
the BA-vs-Subcontractor posture, and the tracked-changes redline mechanics. Come back here when
the BAA is an exhibit to a commercial agreement and the question is how it interacts with the
Order Form, the liability cap, or the order of precedence. Also not for: real-estate loan
closings (use `real-estate-closing-check`), drafting the counterparty's paper, or SOW scoping
from calls (use `sow-builder`).
