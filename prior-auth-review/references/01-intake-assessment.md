...
2. **Requested Service:**
   ...
   - Units Requested per CPT Code: [Units and frequency — e.g., "100 units/week", "64 units/month", "32 units total"]
...
### Step 7b: Unit Calculation & Totals

**REQUIRED:** Always calculate and display total units for the full authorization period.

**Calculate the authorization period:**
```python
auth_start = parse_date(auth_start_date)
auth_end = parse_date(auth_end_date)
total_days = (auth_end - auth_start).days
total_weeks = round(total_days / 7)
total_months = round(total_days / 30)
```

**For each CPT code, normalize units to totals based on how they were provided:**

```python
for code in cpt_codes:
    if units_given_as == "total":
        total_units = given_units  # Already a total, use as-is
    elif units_given_as == "per_week":
        total_units = given_units * total_weeks
    elif units_given_as == "per_month":
        total_units = given_units * total_months
    elif units_given_as == "per_day":
        total_units = given_units * total_days

    total_hours = total_units / 4  # 1 unit = 15 min, 4 units = 1 hour
```

**Display a summary table with:**

| CPT Code | Description | Rate (as given) | Total Units (auth period) | Total Hours |
|----------|-------------|-----------------|--------------------------|-------------|
| [code]   | [desc]      | [X units/week]  | [calculated total]       | [total/4]   |

**Then display:**
- Grand total units across all codes
- Grand total hours across all codes
- Breakdown by category (Assessment, Direct Treatment, Supervision, Caregiver Training)
- Weekly hours total for limit compliance checking

**Check against payer limits (if known):**
- Combined weekly treatment hours vs. payer maximum (e.g., 40 hrs/week for NV Medicaid)
- Supervision ratio (supervision hours / treatment hours) vs. payer cap (e.g., 20%)
- Auth period length vs. payer maximum (e.g., 180 days)

Display PASS/FAIL for each limit check.

**Store calculated totals in the assessment waypoint** (see Step 8 schema).
...
```json
{
  ...
  "request": {
    ...
    "service": {"type": "...", "description": "...", "cpt_codes": [...], "icd10_codes": [...], "auth_period": {"start": "...", "end": "...", "days": 0, "weeks": 0, "months": 0}},
    "units": {
      "per_code": [
        {"code": "...", "description": "...", "rate": "X units/week", "total_units": 0, "total_hours": 0}
      ],
      "grand_total_units": 0,
      "grand_total_hours": 0,
      "weekly_treatment_hours": 0,
      "supervision_ratio_pct": 0,
      "limit_checks": [
        {"check": "...", "limit": "...", "actual": "...", "status": "PASS/FAIL"}
      ]
    },
