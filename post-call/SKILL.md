...
name: post-call
description: >
  Runs after any prospect meeting (Fathom). Two outputs: (1) HubSpot pipeline
...
For every prospect on a Fathom meeting in the target window (default: last 7 days)
...
- Fathom: `list_meetings(created_after=<window start>, max_pages=3)` — filter to last 7 days
...
- Fathom: `mcp__claude_ai_Fathom__list_meetings` / `get_meeting_transcript`
