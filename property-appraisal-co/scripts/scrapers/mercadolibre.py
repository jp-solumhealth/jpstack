...
from ._card_extract import extract_cards_from_page, _parse_card_text

SOURCE = "mercadolibre"
BASE = "https://inmuebles.mercadolibre.com.co"
...
def _search_url(subject: Subject, transaction: str, page_n: int) -> str:
    trans = "venta" if transaction == "venta" else "arriendo"
    city = _slug(subject.city or "bogota-d-c")
    barrio = _slug(subject.barrio) if subject.barrio else ""
    path = f"/apartamentos/{trans}/{city}"
    if barrio:
        path += f"/{barrio}"
    # MercadoLibre paginates with _Desde_ offset
    qs = ""
    if page_n > 1:
        qs = f"_Desde_{(page_n - 1) * 48 + 1}"
...
async def _search(ctx: BrowserContext, subject: Subject, transaction: str, max_results: int) -> list[Listing]:
    ...
    try:
        for n in range(1, 4):
            if not await safe_goto(page, _search_url(subject, transaction, n)):
                break
            await polite_sleep()
